...
const animalsWithThreeLetters = animals
  .filter(pet => pet.length === 3)
...
const animalsNumberOfLetters = animals
  .map(item => item.length)

console.log(animalsNumberOfLetters)  
...
const OddAmount = numbers
  .filter(number => number % 2)
  .reduce((acc, number) => acc + number, 0)
  //.reduce((acc, item) => item % 2 !== 0 ? acc + item : acc, 0)
