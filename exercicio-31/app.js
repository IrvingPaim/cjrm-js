...
const logGitHubUser = async username => console.log(await fetchGitHubUser(username))

logGitHubUser('IrvingPaim')
...
const filter = (array, func) => {
  ...
  const filterItem = (item, index) => {
    const itemShouldBeAdd = func(item, index, array)
    if (itemShouldBeAdd) {
      newArray.push(item)
    }
  }
